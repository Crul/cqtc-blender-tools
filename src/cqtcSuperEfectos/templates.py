...
class AddSuperEfectoTemplateOperator(bpy.types.Operator):
	...
	def execute(self, context):
		...
		if len(old_template) == 0:
			...
			context.scene.super_efecto.template_options.append(new_template_option)
...
class RemoveSuperEfectoTemplateOperator(bpy.types.Operator):
	...
	def execute(self, context):
		template = context.scene.super_efecto.template
		for tmpl in context.scene.super_efecto.template_options:
			if tmpl[0] == template:
				context.scene.super_efecto.template_options.remove(tmpl)
